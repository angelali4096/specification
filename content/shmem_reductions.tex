...
\apidescription{
    ...
    Before any \ac{PE} calls a reduction routine, the implementor must ensure that the
    following conditions are met (synchronization via a \OPR{barrier} or some other
...
\apinotes{  
    ...
    time it is used. The implementor must ensure that the \VAR{pSync} array is not being updated on any \ac{PE}
